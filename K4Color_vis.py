...
def plotGraph():
    plt.scatter(Position[:, 0], Position[:, 1], marker="o", color='black')
    plt.xlabel('x_1')
    plt.ylabel('x_2')
    n, n1 = ColorMat.shape
    assert n == n1
    ...
    for edge in combinations(range(n), 2):
        ...
        if color != 'none':
            plt.plot([Position[i][0], Position[j][0]], [Position[i][1], Position[j][1]], color=color)
    plt.show()
...
for num, edge in enumerate(combinations(range(n), 2)):
    ...
    ColortempW[i, j] = 1
    ColortempW[j, i] = 1
    Ew = ExpWeight(ColortempW)  # compute the expectation conditioned on that edge is colored white
    # try if colored black
    ColortempB = np.copy(ColorMat)
    ColortempB[i, j] = -1
    ColortempB[j, i] = -1
    Eb = ExpWeight(ColortempB)  # compute the expectation conditioned on that edge is colored black
    # De-randomization
